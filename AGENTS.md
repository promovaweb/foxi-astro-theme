...
- **Sincronização de Skills**: Qualquer capacidade (skill) criada ou modificada nos diretórios `.gemini/skills/` ou `.claude/skills/` deve ser replicada no outro diretório para manter a equivalência entre as ferramentas.
...
- **DataLayer Obrigatório**: Todas as páginas `.astro` que utilizam o componente `Layout` DEVEM obrigatoriamente passar a prop `dataLayer` com informações relevantes (tipo de página, título, categoria, etc.) para integração com Analytics/GTM.
- **Formatação de Markdown (OBRIGATÓRIO)**: Ao final de QUALQUER tarefa que crie ou modifique um arquivo `.md`, você **DEVE OBRIGATORIAMENTE** ativar e executar a skill `markdown-format` (Gemini ou Claude). Não finalizar uma tarefa sem garantir que o linting passou.
